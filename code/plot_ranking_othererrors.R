...
my_error <- function(obs,est,dev.type="all"){ #adpated from Gottfried code retro.pm.fun
  ...
  track_sig <- sum(devs/abs(devs),na.rm = TRUE)

  nfm <- sum(devs/(est+obs),na.rm = TRUE)
  
  retro.pm.out <- matrix(NA,nrow=2,ncol=6,
                         dimnames=list(c("RawDev","DevLog"),
                                       c("MRE","MAE","MPE","RMSE","TRSIG","NFM")))

  retro.pm.out["RawDev",] <-  c(mre,mae,mpe,rmse,track_sig,nfm)
  ...
  for (j in 3:3){ #age
    ...
    for (k in 1:1){
      ...
      out<-NULL
          ...
          pop_rank <- out %>%
            select(model,MRE,MAE,MPE,RMSE,TRSIG,NFM) %>%
            # mutate(MRE=round(MRE*1e3)/1e3,
            #        MAE=round(MAE*1e3)/1e3,
            #        MPE=round(MPE*1e3)/1e3,
            #        RMSE=round(RMSE*1e3)/1e3) %>%
            mutate(
                   MRE_rank_bias = rank(MRE),
                   MPE_rank_bias = rank(MPE),
                   MRE_rank=rank(abs(MRE)),
            ...
            mutate(rel_rank_RMSE = RMSE_rank/n(),
                   rel_rank_MRE = MRE_rank/n(),
                   rel_rank_MAE = MAE_rank/n(),
                   rel_rank_MPE = MPE_rank/n(),
...
ggplot(new,aes(x = pname, y = model, fill = rel_rank_MRE)) +
  geom_tile(colour = "white", size = 0.2) +
  scale_fill_gradientn(colours = cols,name = "Abs(MRE)\nRelative Rank",
...
ggsave("../plot/plot_rank_absMRE.png", h = 11, w = 19)

ggplot(new,aes(x = pname, y = model, fill = rel_rank_MPE)) +
  geom_tile(colour = "white", size = 0.2) +
  scale_fill_gradientn(colours = cols,name = "Abs(MPE)\nRelative Rank",
                       breaks = c(min(new$rel_rank_all),0.5,1),labels = c("0 Best Model","0.5","1.0 Worst Model")) +
  theme_bw() +
  scale_y_discrete(limits = rev(model_level))+
  scale_x_discrete(limits = pop_level) +
  ...
  theme(legend.key.height= unit(4, 'cm'))
#theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave("../plot/plot_rank_absMPE.png", h = 11, w = 19)

ggplot(new,aes(x = pname, y = model, fill = rel_rank_MAE)) +
  geom_tile(colour = "white", size = 0.2) +
  scale_fill_gradientn(colours = cols,name = "MAE\nRelative Rank",
                       breaks = c(min(new$rel_rank_all),0.5,1),labels = c("0 Best Model","0.5","1.0 Worst Model")) +
  theme_bw() +
  scale_y_discrete(limits = rev(model_level))+
  scale_x_discrete(limits = pop_level) +
  ...
  theme(legend.key.height= unit(4, 'cm'))
#theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave("../plot/plot_rank_MAE.png", h = 11, w = 19)
...
pname1<-c( "Early Stuart", "Late Stuart","Stellako",
          "Bowron","Raft","Quesnel", "Chilko","Seymour",
          "Late Shuswap", "Birkenhead","Cultus","Portage",
          "Weaver" ,"Upper Barriere(Fennell)","Scotch",
          "Gates","Nadina","Pitt")#,"Harrison")
new$Stock <- pname1[new$pop]
table_publish <- new %>%
  select(Stock, MAE, MRE=MRE, MPE=MPE,RMSE,
         MAE_rank,absMAE_rank = MRE_rank,absMPE_rank = MPE_rank,RMSE_rank,
         Average_rank=average, Overall_rank,
         rel_rank_MAE,rel_rank_absMAE=rel_rank_MRE,
         rel_rank_absMPE=rel_rank_MPE,rel_rank_RMSE)
write.csv(table_publish,file = "../table/TableS3.csv",row.names = FALSE)


ggplot(new,aes(x = pname, y = model, fill = TRSIG)) +
  geom_tile(colour = "white", size = 0.2) +
  geom_text(data = new %>% filter(TRSIG<=2&TRSIG>=-2),aes(label =TRSIG),size = 7) +
  scale_fill_gradientn(colours = cols,name = "Tracking\nSignal",
                       limits=c(-12,12),breaks= c(-12,-7,-2,0,2,7,12)) +
  theme_bw() +
  scale_y_discrete(limits = rev(model_level))+
  scale_x_discrete(limits = pop_level) +
  xlab("")+ylab("")+
  theme(text = element_text(size = 20)) +
  theme(legend.key.height= unit(4, 'cm'))
ggsave("../plot/plot_tracking_signal.png", h = 11, w = 19)

ggplot(new,aes(x = pname, y = model, fill = NFM)) +
  geom_tile(colour = "white", size = 0.2) +
  geom_text(data = new %>% filter(NFM<2&NFM>-2),aes(label = round(NFM, digits = 1)),size = 7) +
  scale_fill_gradientn(colours = cols,name = "NFM",
                       limits=c(-12,12),breaks= c(-12,-7,-2,0,2,7,12)) +
  theme_bw() +
  scale_y_discrete(limits = rev(model_level))+
  scale_x_discrete(limits = pop_level) +
  ...
  theme(text = element_text(size = 20))+
  theme(legend.key.height= unit(4, 'cm'))
ggsave("../plot/plot_NFM.png", h = 11, w = 19)
