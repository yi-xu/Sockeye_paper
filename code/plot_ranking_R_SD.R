...
my_error <- function(obs,est,dev.type="all"){ #adpated from Gottfried code retro.pm.fun
  ...
  par_r <- cor(est,obs,use = "complete.obs")
  par_sd <- sd(est,na.rm = TRUE)/sd(obs,na.rm = TRUE)
  retro.pm.out <- matrix(NA,nrow=2,ncol=6,
                         dimnames=list(c("RawDev","DevLog"),c("MRE","MAE","MPE","RMSE","R","SD")))
  
  retro.pm.out["RawDev",] <-  c(mre,mae,mpe,rmse,par_r,par_sd)
  retro.pm.out["DevLog",] <-  c(mre.log,mae.log,mpe.log,rmse.log,par_r,par_sd)
  ...
  for (j in 3:3){ #age
    ...
    for (k in 1:1){
      ...
      out<-NULL
       ...
       # add forecast
       fc_pop <- fc %>% filter(popID==ipop&retyr%in%2009:2020)
       fc_row <- as_tibble_row(my_error(fc_pop$fc,fc_pop$tot)) %>% mutate(model="Forecast")
       out <- bind_rows(out, fc_row)
          pop_rank <- out %>%
            select(model,R,SD) %>%
...
cols <- brewer.pal(11, 'Spectral')#'RdYlBu')
...
ggplot(new,aes(x = pname, y = model, fill = SD)) +
  geom_tile(colour = "white", size = 0.2) +
  scale_fill_gradientn(colours = cols[1:11],name = "Normalized\nStandard\nDeviation\n",
                       values = c(1.0, 0.66,0),
                       limits = c(0,1.55),breaks = c(0,0.5,1.0,1.5,1.55),
                       labels = c("0","0.5","1.0","1.5",">1.5"),
                       na.value = cols[1]) +
  theme_bw() +
  scale_y_discrete(limits = rev(model_level))+
  scale_x_discrete(limits = pop_level) +
